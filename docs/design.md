...
```text
Browser microphone / future phone call
        |
        v
Container Apps hosted SPA or ACS call ingress
        |
        v
Container Apps API -> Azure AI Speech STT -> transcript stream
        |
        v
Container Apps API
  - session state
...
| リソース | 用途 |
| --- | --- |
| Azure Container Apps | SPA と会話オーケストレーター API をホストする。WebSocket/HTTP で音声・transcript ストリームを受ける。 |
...
| Azure AI Speech | API 側の managed identity でリアルタイム STT を提供する。 |
| Azure Key Vault | DB パスワード、Speech キー、OpenAI キーなどを保存する。 |
| User-assigned managed identity | Container Apps API が Azure OpenAI、Azure AI Speech、Key Vault へアクセスする。 |
| Log Analytics | Container Apps のログを保持する。 |

Azure Communication Services は電話番号・通話制御の本番入口として設計に含めるが、電話番号取得や通信規制に依存する作業は PoC の `azd up` 必須経路から外す。開発・デバッグ時は Container Apps で配信する SPA を開き、ブラウザの `getUserMedia()` で取得した PCM 音声を API の WebSocket に送る。Azure Speech への接続と認証は API 側に閉じ込め、ブラウザへ Speech key/token を配布しない。
...
src/AiCallCenter.Api
  ...
  /ws/conversations/{id}
  /ws/audio/conversations/{id}
```

API は音声処理の入口を抽象化する。ブラウザ SPA と将来の ACS ingress は、どちらも API に音声または transcript chunk を送り、API 側で「transcript chunk」と「utterance finalized」に正規化する。API 側は音声デバイスや電話回線に依存せず、Azure Speech STT、HorizonDB、応答選択に集中する。
...
- Container Apps API の managed identity に Azure AI Speech User と Azure OpenAI User を付与する。
...
- HorizonDB 管理者パスワードは Key Vault に保存する。Speech/OpenAI は local auth disabled を前提に managed identity で呼び出す。
- Container Apps は User-assigned managed identity を使い、Key Vault 参照で secret を取得する。
- ブラウザは UI とマイク入力のみを担当し、Azure Speech の credential は保持しない。
...
| 電話番号取得は完全自動化できない可能性 | `azd up` 必須経路はブラウザ SPA PoC とし、ACS は拡張入口として分離する。 |
| 音声ストリーム処理が環境差の影響を受ける | ブラウザは PCM chunk 送信のみを担当し、Azure Speech SDK と認証は Container Apps API 側に集約する。 |
